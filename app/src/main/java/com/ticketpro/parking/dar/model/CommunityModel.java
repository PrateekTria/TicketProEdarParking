...
@Entity(tableName = "community_meeting")
public class CommunityModel {
    ...
    @ColumnInfo(name = "comment_group")
    @SerializedName("comment_group")
    @Expose
    ...
    public static long getNextPrimaryId() {
        long nextId = 0;
        nextId = ParkingDatabase.getInstance(TPApplication.getInstance()).communityModelDao().getNextPrimaryId();
        return nextId + 1;
    }
    public static List<CommunityModel> _getList() throws Exception{
        return ParkingDatabase.getInstance(TPApplication.getInstance()).communityModelDao().getCommunityModel();
    }
    public static void removeAll() throws Exception {
        ParkingDatabase.getInstance(TPApplication.getInstance()).communityModelDao().removeAll();
    }

    public static void removeById(int id) throws Exception {
        ParkingDatabase.getInstance(TPApplication.getInstance()).communityModelDao().removeById(id);
    }

   /* public static void insertCommunityModel(CommunityModel model) {
        new InsertCommunityModel(model).execute();
    }*/

    private static class InsertCommunityModel extends AsyncTask<Void,Void,Void> {
        private CommunityModel model;
        public InsertCommunityModel(CommunityModel model) {
            this.model = model;
        ...
        protected Void doInBackground(Void... voids) {
            ParkingDatabase.getInstance(TPApplication.getInstance()).communityModelDao().insertCommunityModel(model);
            return null;
        }
    }
    ...
    public static void insertCommunityModel(CommunityModel model) {
        ...
        instance.communityModelDao().insertCommunityModel(model).subscribeOn(Schedulers.io()).subscribeWith(new CompletableObserver() {
            @Override
