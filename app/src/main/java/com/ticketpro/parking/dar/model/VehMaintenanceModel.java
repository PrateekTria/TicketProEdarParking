...
/*public class VehMaintenanceModel {
}*/
@Entity(tableName = "veh_maintenance")
public class VehMaintenanceModel {
    ...
    @ColumnInfo(name = "senior_advised")
    @SerializedName("senior_advised")
    @Expose
    private String seniorAdvised;
    @ColumnInfo(name = "vehicle_change")
    @SerializedName("vehicle_change")
    @Expose
    private String vehicleChanged;
    ...
    public static long getNextPrimaryId() {
        long nextId = 0;
        nextId = ParkingDatabase.getInstance(TPApplication.getInstance()).vehMaintenanceDao().getNextPrimaryId();
        return nextId + 1;
    }
    public static List<VehMaintenanceModel> _getList() throws Exception{
        return ParkingDatabase.getInstance(TPApplication.getInstance()).vehMaintenanceDao().getVehMaintenanceModel();
    }

    public static void removeAll() throws Exception {
        ParkingDatabase.getInstance(TPApplication.getInstance()).vehMaintenanceDao().removeAll();
    }

    public static void removeById(int id) throws Exception {
        ParkingDatabase.getInstance(TPApplication.getInstance()).vehMaintenanceDao().removeById(id);
    }
   ...
   /* public static void insertOffStreetModel(OffStreetModel model) {
    ...
    public static void insertVehMaintenanceModel(VehMaintenanceModel model) {

        final ParkingDatabase instance = ParkingDatabase.getInstance(TPApplication.getInstance());
        long time = System.currentTimeMillis();

        instance.vehMaintenanceDao().insertVehMaintenanceModel(model).subscribeOn(Schedulers.io()).subscribeWith(new CompletableObserver() {
            @Override
    ...
    public String getSeniorAdvised() {
        return seniorAdvised;
    }
    ...
    public void setSeniorAdvised(String seniorAdvised) {
        this.seniorAdvised = seniorAdvised;
    }

    public String getVehicleChanged() {
        return vehicleChanged;
    }

    public void setVehicleChanged(String vehicleChanged) {
        this.vehicleChanged = vehicleChanged;
    }
