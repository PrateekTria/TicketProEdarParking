...
public class TrainingModel {
    ...
    public static List<TrainingModel> _getList() throws Exception{
        return ParkingDatabase.getInstance(TPApplication.getInstance()).trainingModelDao().getTrainingModel();
    }
    public static void removeAll() throws Exception {
        ParkingDatabase.getInstance(TPApplication.getInstance()).trainingModelDao().removeAll();
    }
    public static long getNextPrimaryId() {
        long nextId = 0;
        nextId = ParkingDatabase.getInstance(TPApplication.getInstance()).trainingModelDao().getNextPrimaryId();
        return nextId + 1;
    }
    public static void removeById(int id) throws Exception {
        ParkingDatabase.getInstance(TPApplication.getInstance()).trainingModelDao().removeById(id);
    }

   /* public static void insertTrainingModel(TrainingModel model) {
        new InsertTrainingModel(model).execute();
    }*/

    private static class InsertTrainingModel extends AsyncTask<Void,Void,Void> {
        private TrainingModel model;
        public InsertTrainingModel(TrainingModel model) {
            this.model = model;
        ...
        protected Void doInBackground(Void... voids) {
            ParkingDatabase.getInstance(TPApplication.getInstance()).trainingModelDao().insertTrainingModel(model);
            return null;
        }
    }
    ...
    public static void insertTrainingModel(TrainingModel model) {
        ...
        instance.trainingModelDao().insertTrainingModel(model).subscribeOn(Schedulers.io()).subscribeWith(new CompletableObserver() {
            @Override
