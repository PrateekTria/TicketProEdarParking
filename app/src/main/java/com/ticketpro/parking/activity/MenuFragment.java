...
*/
		...
		MenuAction m363=new MenuAction();
		if(TPApplication.getInstance().enableSpPlus){
			m363.setTitle("Turn Off SpPLus");
		}else{
			m363.setTitle("Turn On SpPlus");
		}
		...
		if(Feature.isFeatureAllowed(Feature.PARK_SP_PLUS)){
			extrasMenu.getSubMenus().add(m363);
		}
