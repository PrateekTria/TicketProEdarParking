...
import com.ticketpro.parking.syncbackup.syncactivity.NewSyncBackup;
import com.ticketpro.parking.syncreport.requestmodel.Detail;
import com.ticketpro.parking.syncreport.requestmodel.SyncReport_Json_rpc;
import com.ticketpro.parking.syncreport.requestmodel.SyncReport_Req_Params;
...
public class HomeActivity extends BaseActivityImpl {
    ...
    public void onCreate(Bundle savedInstanceState) {
        try {
            ...
            sharedPreferences= getSharedPreferences("EDAR",Context.MODE_PRIVATE);
            pref_UserId= sharedPreferences.getString("REF_USERID", "0");
            ...
            SharedPreferences sharedPreferences = getSharedPreferences("EDAR", Context.MODE_PRIVATE);
 ...
 */
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        SpannableString s = new SpannableString("Test Network");
        s.setSpan(new ForegroundColorSpan(Color.WHITE), 0, s.length(), 0);
        item.setTitle(s);

        item = menu.getItem(6);
        s = new SpannableString("Backup Database");
        s.setSpan(new ForegroundColorSpan(Color.WHITE), 0, s.length(), 0);
        item.setTitle(s);
        ...
        if (Feature.isSystemFeatureAllowed(Feature.DAR_REPORT_SYNC)) {
            item = menu.getItem(1);
            s = new SpannableString("Create eDar report");
            s.setSpan(new ForegroundColorSpan(Color.WHITE), 0, s.length(), 0);
            item.setTitle(s);
...
//            JobUtil.scheduleJob(HomeActivity.this);
    ...
    public void createReportRequest() throws IOException {
        Date currentDate = new Date();
        SimpleDateFormat dateFormat1 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault());
        String formattedDate = dateFormat.format(currentDate);
        try {
            dutyReports = DutyReport.getDutyRecord(formattedDate);
        } catch (TPException e) {
            e.printStackTrace();
        }
        if (dutyReports.isEmpty()) {
            return;
        }
        ...
        for (DutyReport report : dutyReports) {
            ...
            detail.setDutyId(report.getDutyId());
            detail.setDateInTime(dateFormat1.format(report.getDateIn()));
            detailsList.add(detail);
        }
        SyncReport_Json_rpc jsonRpc = new SyncReport_Json_rpc();
        SyncReport_Req_Params param = new SyncReport_Req_Params();
        param.setUserId(Integer.parseInt(pref_UserId));
       ...
       // System.out.println("RequestSync**" + new Gson().toJson(jsonRpc));
        log.info("darReportRequest-->>" + new Gson().toJson(jsonRpc));
        if (isNetworkConnected()) {
            try {
                ApiRequest api = ServiceGenerator.createRxService(ApiRequest.class);
                api.syncReport(jsonRpc).enqueue(new Callback<SyncReport_Response>() {
                    @Override
                    public void onResponse(Call<SyncReport_Response> call, Response<SyncReport_Response> response) {
                        if (response.isSuccessful()) {

                            if (progressDialog.isShowing()){
                                progressDialog.dismiss();
                            }
                                if (response.body().getResult().getMsg()) {
                                    log.info("Report synced successfully");
                                    Toast.makeText(HomeActivity.this,"Report synced successfully ",Toast.LENGTH_SHORT).show();

                                } else {
                                    log.error("darReportRequest--->>Report synced failed-1");
                            ...
                            log.error("darReportRequest--->>Report synced failed-3");

                            Toast.makeText(HomeActivity.this,"Service not responding",Toast.LENGTH_SHORT).show();

                        }
                    }

                    @Override
                    public void onFailure(Call<SyncReport_Response> call, Throwable t) {

                        if (progressDialog.isShowing()){
                            progressDialog.dismiss();
                        }
                        log.error("darReportRequest--->>"+t.getMessage().toString());
                ...
                e.printStackTrace();
                log.error("darReportRequest--->>"+e.getMessage().toString());
                Toast.makeText(HomeActivity.this,"Service not responding",Toast.LENGTH_SHORT).show();
            ...
            log.error("darReportRequest--->>Report synced failed-6");

            Toast.makeText(HomeActivity.this,"Please check your internet connecticity",Toast.LENGTH_SHORT).show();
