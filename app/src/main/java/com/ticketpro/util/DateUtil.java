...
public class DateUtil {
    ...
    public static Date spPlusTime(String utcString) {
        ...
        try {
            ...
            String date = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss aa").format(format);
    ...
    public static Date getPP2DateFromUTCString(String utcString) {
        ...
        /*try {
           // @SuppressLint({"NewApi", "LocalSuppress"}) DateTimeFormatter formatter = DateTimeFormatter.ISO_DATE_TIME;

            @SuppressLint({"NewApi", "LocalSuppress"}) ZonedDateTime utcDateTime = ZonedDateTime.parse(utcString, DateTimeFormatter.ISO_DATE_TIME);

            // Convert UTC to PST (Pacific Standard Time)
            @SuppressLint({"NewApi", "LocalSuppress"}) ZoneId pstZoneId = ZoneId.of("America/Los_Angeles");
            @SuppressLint({"NewApi", "LocalSuppress"}) ZonedDateTime pstDateTime = utcDateTime.withZoneSameInstant(pstZoneId);

            // Format and print the converted date-time
            @SuppressLint({"NewApi", "LocalSuppress"}) DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss z");
            @SuppressLint({"NewApi", "LocalSuppress"}) String formattedDateTime = pstDateTime.format(formatter);
            System.out.println("UTC: " + utcDateTime);
            System.out.println("PST: " + formattedDateTime);
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss z");

            Date date = dateFormat.parse(utcString);
...
*/
    ...
    public static String getCurrentTimeTow() {
        try {
            Date date = new Date();
    ...
    public static String getCurrentDateTimeMillisImage() {
        try {
            Date date = new Date();
    ...
    public static boolean isTimeAfter(String strTime, String enTime) throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat("hh:mm a");
        Date startTime = sdf.parse(strTime);
        Date endTime = sdf.parse(enTime);

        return !endTime.before(startTime);
    }

    public static boolean isCurrentDateBig(String strTime, String enTime) throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date startTime = sdf.parse(strTime);
        Date endTime = sdf.parse(enTime);
    ...
    public static boolean isCurrentDateBig1(String strTime, String enTime) throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Date startTime = sdf.parse(strTime);
        Date endTime = sdf.parse(enTime);
