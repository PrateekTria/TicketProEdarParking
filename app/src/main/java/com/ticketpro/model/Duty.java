...
public class Duty implements Serializable {
    ...
    public static ArrayList<Duty> getDutiesList(int idAsg) throws Exception {
        ArrayList<Duty> list = new ArrayList<Duty>();
        list = (ArrayList<Duty>) ParkingDatabase.getInstance(TPApplication.getInstance()).dutiesDao().getDutiesList(idAsg);
        return list;
    }
    ...
    public static Completable insertDuty(final Duty Duty) {
        final ParkingDatabase database = ParkingDatabase.getInstance(TPApplication.getInstance());
        return Completable.fromAction(new Action() {
            @Override
            public void run() throws Exception {
                database.dutiesDao().insertDuty(Duty).subscribeOn(Schedulers.io()).subscribe();
            }
        });
        //new Duty.InsertDuty(Duty).execute();
    }
