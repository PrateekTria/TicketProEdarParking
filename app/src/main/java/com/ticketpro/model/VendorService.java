...
public class VendorService {
    ...
    @Ignore
    public static final String SP_PLUS = "SP_Plus_Plate";
    @Ignore
    public static final String SP_PLUS_ZONE_LIST_INFO = "SP_Plus_ZoneList_info";
    ...
    //Code by Sanjiv for cale service
    public static VendorServiceConfig getServiceConfigSpPlus(String serviceName, int deviceId, String paramChar) {
        VendorServiceConfig config = null;
        VendorService service;
        try {
            service = VendorService.getServiceByName(serviceName);
            ...
            if (service != null) {
                ...
                if (securityKey != null && !securityKey.isEmpty() && !securityKey.equalsIgnoreCase("null")) {
                    //Parse HTTP Authentication USERNAME and PASSWORD
                    if (securityKey.contains("vendor-name")) {
                        String[] authTokens = securityKey.split(",");
                        for (String token : authTokens) {
                            String[] paramTokens = token.split("=");
                            if (paramTokens.length == 2) {
                                if (paramTokens[0].equalsIgnoreCase("vendor-name")) {
                                    config.setUsername(paramTokens[1]);
                                    config.getParamsMap().put("vendor-name", paramTokens[1]);
                                } else if (paramTokens[0].equalsIgnoreCase("x-api-key")) {
                                    config.setPassword(paramTokens[1]);
                                    config.getParamsMap().put("x-api-key", paramTokens[1]);
                                } else {
                                    if (params.length() > 0) {
                                        params.append(paramChar);
                                    }
                ...
                String userParams = registration.getParamMappings();
                if (userParams != null && !userParams.isEmpty()) {
                    if (userParams.contains("vendor-name")) {
                        String[] authTokens = userParams.split(",");
                        for (String token : authTokens) {
                            String[] paramTokens = token.split("=");
                            if (paramTokens.length == 2) {
