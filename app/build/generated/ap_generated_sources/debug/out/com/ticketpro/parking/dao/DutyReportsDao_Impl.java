...
public final class DutyReportsDao_Impl implements DutyReportsDao {
  ...
  public DutyReportsDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfDutyReport = new EntityInsertionAdapter<DutyReport>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR REPLACE INTO `duty_reports` (`report_id`,`userid`,`custid`,`duty_id`,`date_in`,`date_out`,`signature`,`device_id`,`duty_report_id`,`sync_status`,`date_string`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?,?,?)";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, DutyReport value) {
        ...
        if (value.getDate_string() == null) {
          stmt.bindNull(11);
        } else {
          stmt.bindString(11, value.getDate_string());
        }
      }
    };
  ...
  public List<DutyReport> getDutyReports(final int userId) {
    ...
    try {
      ...
      final int _cursorIndexOfDateString = CursorUtil.getColumnIndexOrThrow(_cursor, "date_string");
      ...
      while(_cursor.moveToNext()) {
        ...
        final String _tmpDate_string;
        if (_cursor.isNull(_cursorIndexOfDateString)) {
          _tmpDate_string = null;
        } else {
          _tmpDate_string = _cursor.getString(_cursorIndexOfDateString);
        }
        _item.setDate_string(_tmpDate_string);
  ...
  public DutyReport getDutyReportByPrimaryKey(final String primaryKey) {
    ...
    try {
      ...
      if(_cursor.moveToFirst()) {
        ...
        _result.setDate_string(_tmpDate_string);
      } else {
        _result = null;
      }
      return _result;
  ...
  @Override
  public List<DutyReport> getAllDutyReportsForCurrentDate(final String formattedDate) {
    final String _sql = "SELECT * FROM duty_reports WHERE date_string = ?";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    if (formattedDate == null) {
      _statement.bindNull(_argIndex);
    } else {
      _statement.bindString(_argIndex, formattedDate);
    }
    __db.assertNotSuspendingTransaction();
    final Cursor _cursor = DBUtil.query(__db, _statement, false, null);
    try {
